...
def analyze_nights (table, threshold=0.1, sigma=3) :
    '''
    A function that finds what fraction of stars count as 
    "disked" on each night, and selects outliers.

    Takes a while to run. (1 minute 55 seconds on my machine)

    Hacked on some things to deal with the fact that chipset #4 is missing 
    ...
    disk_counter = np.zeros(( 4, n_nights ))
    # so we'll reference disk_counter[i,j]
    ...
    for i in range(4):
        ...
        for (j, time) in zip(range(n_nights), divvied[i]):
            ...
            n_disks = len(good.where( good.JMHPNT < (1.71*good.HMKPNT) 
                                      - (threshold + sigma * good.HMKPNTERR) ))

            n_total = len(good)
            
            try:
                disk_frac = (1.*n_disks) / n_total
            except:
                disk_frac = 0
            
#            print disk_frac

            disk_counter[i,j] = disk_frac


    # For the purposes of counting "extended" disked stars, exclude 
    #really bright guys (who are known to have ridiculous systematics) 
    #and all ppErrBit-affected sources.
    print "made it this far"
            

    # Really, we should return the number of disks and number of total stars in each analysis, as well as the timestamps
    return (divvied, disk_counter)


def remove_nights (table, analysis):
    '''
    Removes nights from a table based on the output 
    of analyze_nights.

    Must be called after analyze_nights.
    ...
    # First, let's relabel analyze_nights' output.
    
    nights, disk_counter = analysis #this depends on what analyze_nights outputs

    master_cloudy = []
    
    for night_set, disk_set in zip(nights, disk_counter):
        ...
        middle = rb.meanr(disk_set)
        spread = rb.stdr(disk_set)

        cloudy = night_set[ np.abs(disk_set - middle) > 3*spread ]
